...
export default function TypesPage() {
  ...
  const [relatedAddonList, seterelatedAddonList] = useState<any>([]);
  const { price: totalPrice } = usePrice({
    amount: total,
  });
  const { closeCartSidebar,openCartSidebar,displayCartSidebar } = useUI();
  const { addSubscriptionToCart, items } = useCart();
  ...
  useEffect(() => {
    ...
    GetFunction('/pabbly/subscriptions-and-related-addons').then((result) => {
      if (result?.success) {
        ...
        setSelectedData(result?.data?.customer_subscriptions[0]?.plan);
        seterelatedAddonList(result?.data?.customer_subscriptions[0].addons);
    ...
    <>
      {isSubscription ? (
        <>
          ...
          <div className="">
            <div className="">Subscribed Addons</div>
            <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 3xl:grid-cols-6">
              {relatedAddonList?.map((addon: any, index: any) => (
                <div
                  key={index}
                  className="cart-type-neon mt-3  overflow-hidden rounded border-border-200 bg-light"
                >
                  <header className="p-3 md:p-6">
                    ...
                    <>
                      <div className="flex bg-black text-white px-2 rounded py-2 flex-1 items-center justify-center text-sm font-semibold text-heading">
                        {addon.quantity}
                      </div>
                      {/* <Counter
          ...
          <div className=" mt-3">
            <div className="">Related Addons</div>
            <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 3xl:grid-cols-6">
              {addonList?.map((addon: any, index: any) => (
                <ProductCard planData={selectedData} key={index} item={addon} />
              ))}
            </div>
          </div>
          <div onClick={inDrawerClick}>
            <CartCounterButton
              data={isSubscription ? 'Update Subscription' : 'Add Subscription'}
            />
          </div>
          <Drawer
          ...
          >
            <DrawerWrapper hideTopBar={true}>
              <SubscriptionCart
                data={packagesList}
                text={
                  isSubscription ? 'Update Subscription' : 'Add Subscription'
                }
              />
            </DrawerWrapper>
          </Drawer>
