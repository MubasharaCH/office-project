...
import CreateOrUpdateLocationForm from '@/components/location/location-form';
...
export default function UpdateTypePage() {
  ...
  useEffect(() => {
    setloadingData(true);
    GetSpecificFunction('/business-location', query.locationSlug as string).then(
      (result) => {
        if (result) {
  ...
  return (
    <>
      ...
      <CreateOrUpdateLocationForm initialValues={ListData[0]} />
    </>
  );
}
