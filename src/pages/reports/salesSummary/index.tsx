...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    ...
    setloadingData(true);
    GetFunction(
      '/sales-summary-report?start_date=' + sDate + '&end_date=' + eDate
    ).then((result) => {
      ...
      let newProductList = result?.tableData?.map((item: any) => {
        return {
          Date: item?.DateOnly,
          Gross_Sale: item?.GrossSales,
          Net_Sale: item?.NetSales,
          Refund_Sale: item?.RefundSales,
          Discount_Sale: item?.DiscountSales,
        };
      });
  ...
  const onChange = (item) => {
    ...
    if (start && end != 'Invalid date') {
      ...
      setTableLoader(true);
      GetFunction(
        '/sales-summary-report?start_date=' + NewStart + '&end_date=' + NewEend
      ).then((result) => {
      ...
      <Card className="mb-8 flex flex-col items-center xl:flex-row">
        <div className="mb-4 md:w-1/4 xl:mb-0">
          <h1 className="text-xl font-semibold text-heading">
            {t('table:table-item-sales-summary')}
          </h1>
        </div>
              ...
              <Menu.Item>
                {({ active }) => (
                  <div>
                    ...
                    >
                      <DownloadIcon className="w-5 shrink-0" />
                      <Pdf targetRef={tableRef.current} filename="SalesSummaryReport.pdf" options={options}>
                        {({ toPdf }) => (
