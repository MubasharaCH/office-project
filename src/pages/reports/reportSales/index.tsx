...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    ...
    setEndDate(eDate);
    setloadingData(true);
    GetFunction('/sale-report?start=' + sDate + '&end=' + eDate).then(
      (result) => {
        ...
        let newProductList = result?.data?.map((item: any) => {
          return {
            ...
            Payment_Method: item?.payment_methods,
          };
        });
  ...
  const onChange = (item) => {
    ...
    if (start && end != 'Invalid date') {
      ...
      setTableLoader(true);
      GetFunction('/sale-report?start=' + NewStart + '&end=' + NewEend).then(
        (result) => {
      ...
      <Card className="mb-8 flex flex-col items-center justify-between md:flex-row">
        <div className="mb-4 md:w-1/4 xl:mb-0">
          <h1 className="text-xl font-semibold text-heading">
            {t('table:table-item-sales-report')}
          </h1>
        </div>
              ...
              <Menu.Item>
                {({ active }) => (
                  <div>
                    ...
                    <ExportCSV csvData={exportData} fileName="Sales Report" />
                    {/* <DownloadTableExcel
                      filename="Excel File"
                      sheet="users"
                      currentTableRef={tableRef.current}
                    >
                      <div className="flex pl-5">
                        <DownloadIcon className="w-5 shrink-0" />
                        <button className='pl-3 text-sm text-slate-500'> Export excel </button>
                      </div>
                    </DownloadTableExcel> */}
