...
import ProductSalesList from '@/components/productSales/productSales-list';
...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    ...
    setloadingData(true);
    GetFunction(
      '/product-sales-report?start_date=' + sDate + '&end_date=' + eDate
    ).then((result) => {
      ...
      let newProductList = result?.tableData?.map((item: any) => {
        return {
          ...
          Sold_Item: item?.itemsold,
          Net_Sale: item?.netsales,
        };
      });
  ...
  const onChange = (item) => {
    ...
    if (start && end != 'Invalid date') {
      ...
      setTableLoader(true);
      GetFunction(
        '/product-sales-report?start_date=' + NewStart + '&end_date=' + NewEend
      ).then((result) => {
        setDummyArr(result.tableData);
        // console.log(result.tableData,'product sales data');
        setNewArr(result.tableData);
        let newProductList = result?.tableData?.map((item: any) => {
          return {
            SKU: item?.productSku,
              ...
              <Menu.Item>
                {({ active }) => (
                  <div>
                    ...
                    >
                      <DownloadIcon className="w-5 shrink-0" />
                      <Pdf targetRef={tableRef.current} options={options} filename="productSales.pdf">
                        {({ toPdf }) => (
