...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    ...
    setloadingData(true);
    GetFunction(
      '/sales-category-report?start_date=' + sDate + '&end_date=' + eDate
    ).then((result) => {
  ...
  const onChange = (item) => {
    ...
    if (start && end != 'Invalid date') {
      ...
      setTableLoader(true);
      GetFunction(
        '/sales-category-report?start_date=' + NewStart + '&end_date=' + NewEend
      ).then((result) => {
              ...
              <Menu.Item>
                {({ active }) => (
                  <div>
                    ...
                    >
                      <DownloadIcon className="w-5 shrink-0" />
                      <Pdf targetRef={tableRef.current} options={options} filename="categorySales.pdf">
                        {({ toPdf }) => (
