...
import StockList from '@/components/stockTransfer/stock-transfer-list';
...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    setloadingData(true);
    GetFunction('/stock-transfer/list').then((result) => {
      ...
      <Card className="mb-8 flex flex-col items-center xl:flex-row">
        <div className="mb-4 md:w-1/4 xl:mb-0">
          <h1 className="text-xl font-semibold text-heading">
            {t('table:table-label-stock-transfer')}
          </h1>
        </div>
        ...
        <div className="flex w-full flex-col items-center space-y-4 ms-auto md:flex-row md:space-y-0 xl:w-1/2">
          ...
          <LinkButton
            href={Routes.stockTransfer.create}
            className="h-12 w-full md:w-auto md:ms-6"
          >
            <span className="block md:hidden xl:block">
              + {t('table:label-add-stock-transfer')}
            </span>
