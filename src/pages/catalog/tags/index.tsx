...
export default function Tags() {
  ...
  permissionList?.forEach((item) => {
    ...
    if (lowercaseItem.includes('tag.create')) {
      tagPermission.create = true;
    }
    if (lowercaseItem.includes('tag.update')) {
      tagPermission.update = true;
    }
    if (lowercaseItem.includes('tag.delete')) {
      tagPermission.delete = true;
    }
  });
      ...
      <Card className="mb-8 flex flex-col items-center xl:flex-row">
        ...
        <div className="ms-auto flex w-full flex-col items-center space-y-4 md:flex-row md:space-y-0 xl:w-1/2">
          ...
          locale === Config.defaultLanguage && (
            <LinkButton
              href={`/catalog${Routes.tag.create}`}
