...
export default function TypesPage() {
  ...
  useEffect(() => {
    setloadingData(true);
    GetFunction('/sell?status=draft').then((result) => {
  ...
  React.useEffect(() => {
    if (metaData != undefined) {
      setloadingData(true);
      GetFunction('/sell?status=draft&&per_page=' + metaData?.total).then(
        (result) => {
          setMetaData(result.meta);
    ...
    <>
      <Card className="mb-8 flex flex-col items-center xl:flex-row">
        <div className="mb-4 md:w-1/4 xl:mb-0">
          <h1 className="text-xl font-semibold text-heading">{t('Drafts')}</h1>
        </div>
