...
export default function TypesPage() {
  ...
  React.useEffect(() => {
    GetFunction( '/contactapi?type=customer'+ '&per_page=' + listPerPage).then((result) => {
      if (result) {
  ...
  const ChangePagination = (current) => {
    setTableLoader(true);
    GetFunction('/contactapi?type=customer&name='+query+'&email='+query+'&page=' + current+ '&per_page=' + listPerPage).then((result) => {
      if (result) {
        setMetaData(result.meta);
        setListData(result.data);
        setNewArr(result.data);
  ...
  const filterBySearch = (event) => {
    ...
    GetFunction( '/contactapi?type=customer&name='+event.target.value+'&email='+event.target.value+'&per_page=' + listPerPage).then((result) => {
      if (result) {
      setMetaData(result.meta);
      setListData(result.data);
      setNewArr(result.data);
