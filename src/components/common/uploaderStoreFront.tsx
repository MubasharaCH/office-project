...
import { ACCEPTED_FILE_TYPES_StoreFront } from '@/utils/constants';
...
export default function Uploader({
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...(!acceptFile ? { accept: 'video/*' } : { accept: ACCEPTED_FILE_TYPES_StoreFront }),
    multiple,
    onDrop: async (acceptedFiles) => {
      if (acceptedFiles.length) {
        if (multiple) {
          setImageFile((current) => [...current, acceptedFiles[0]]);
        } else {
