...
const CouponList = ({
  ...
  const columns = [
    {
      title:<span style={{fontFamily:'poppins'}}>{t('table:table-item-id')}</span> ,
      dataIndex: 'id',
      key: 'id',
      align: 'center',
      width: 120,
    ...
    {
      title:<span style={{fontFamily:'poppins'}}>{t('table:table-item-banner')}</span> ,
      dataIndex: 'image',
      key: 'image',
      width: 74,
    ...
    },
    {
      ...
      render: (text: string) => (
        <span className="whitespace-nowrap" style={{fontFamily:'poppins'}}>{text}</span>
      ),
    },
    {
      title: (
      ...
      render: function Render(amount: number, record: any) {
        ...
        if (record.type === 'PERCENTAGE_COUPON') {
          return <span style={{fontFamily:'poppins'}}>{amount}%</span>;
        }
        return <span style={{fontFamily:'poppins'}}>{price}</span>;
      },
    },
    {
      title: (
