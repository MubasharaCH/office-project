...
const UnitList = (list: any) => {
  ...
  const columns = [
    ...
    },
    {
      title: (
        <span style={{ fontFamily: 'poppins' }}>
          {t('table:table-item-gross-sales')}
        </span>
      ),
      className: 'cursor-pointer',
      ...
      render: (GrossSales: any) => (
        <span style={{ fontFamily: 'poppins' }}>
          {' '}
          {/* className="whitespace-nowrap"{GrossSales}  */}
          {GrossSales &&
            list.BusinesDetails.symbol + Number(GrossSales).toLocaleString()}
        </span>
      ),
    },
    {
